...
private:
    struct vertex {
        ...
        int edges[26];     // массив сыновей
        int fast_move[26];    // массив переходов (запоминаем переходы в ленивой рекурсии)
    ...
    void add_pattern(const std::pair<int, int> &submask_pos, int patt_index);
    ...
    std::string pattern;      // шаблон по которой ищем вхождения
...
aho_corasik_automat::aho_corasik_automat(const std::string &patt)
    ...
    for (int i = 0; i < subpatt_positions.size(); i++) {
        add_pattern(subpatt_positions[i], i);   // поместили шаблон в бор
    }
}


void aho_corasik_automat::add_pattern(const std::pair<int, int> &submask_pos, int patt_index) {
    int current_vertex = 0;
    for (int i = submask_pos.first; i <= submask_pos.second; i++) {
        char character = pattern[i] - 'a';
...
void aho_corasik_automat::find_sub_pattern_pos(const std::string &patt) {
    std::pair<int, int> current_subpatternpos;      // нач и конец шаблона между ?...?
    /*if (isalpha(patt[0])) {             // a-z, A-Z ?
        current_subpatternpos.first = 0;
    }
    if (pattern[1] == '?' && isalpha(pattern[0])) {
        current_subpatternpos.second = 0;
        subpatt_positions.push_back(current_subpatternpos);
    }*/

    for (int i = 1; i < pattern.length() - 1; i++) {
        if (patt[i - 1] == '?' && isalpha(patt[i])) {
            current_subpatternpos.first = i;
        }
        if (patt[i + 1] == '?' && isalpha(patt[i])) {
            current_subpatternpos.second = i;
            subpatt_positions.push_back(current_subpatternpos);
        }
    }

    if (pattern[patt.length() - 2] == '?' && isalpha(patt[patt.length() - 1])) {
        current_subpatternpos.first = (int) patt.length() - 1;
    }
    if (isalpha(pattern[pattern.length() - 1])) {
        current_subpatternpos.second = (int) (patt.length() - 1);
        subpatt_positions.push_back(current_subpatternpos);
    }
}
...
std::vector<int> aho_corasik_automat::find_matches(const std::string &text) {
    ...
    for (int i = 0; i < text.length(); i++) {
        vertex = get_link(vertex, text[i] - 'a');
            ...
            if (bohr[curr_vertex].is_pattern) {     // нашли нужный нам шаблон (его начало)
                for (int index : bohr[curr_vertex].indices_of_pattern) {
                    ...
                    if ((start_index - subpatt_positions[index].first >= 0) &&              // совпадение шаблона
                        (start_index - subpatt_positions[index].first + pattern.length() - 1 < text.length()))
